...
def create_station
  ...
  @stations_user_intermediate_all << station_user
  menu
end
...
def delete_station_rout
  ...
  @routes_user_all[index_user_route].intermediate_stations.each_with_index do |x, index|
  print index
  print ": "
  puts x.name
  end
  index_user_station = gets.chomp.to_i
  index_user_station += 1
...
def delete_wagon_train
  ...
  @trains_user_all[index_user_train].wagon.each_with_index do |x, index|
  print index
  print ": "
  puts x
  end
  ...
  else puts "У этого поезда нет вагонов"
  end
